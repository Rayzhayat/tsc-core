...
class Pemasukan extends CI_Controller
{
    ...
    public function proses_tambah()
    {
        ...
        // 🔥 CRITICAL FIX: JOURNAL ENTRY DENGAN OCLY SUPPORT
        $entries = [];

        // 1. DEBIT: Bank (uang masuk)
        $entries[] = [
            'akun_id' => $akun_bank_id,
            'debit' => $total_diterima,  // Yang diterima di bank
            'kredit' => 0
        ];

        // 2. CREDIT: Pendapatan (nominal pokok)
        $entries[] = [
        ...
        // 3. CREDIT: PPN Keluaran (OCLY 53) - jika ada PPN
        if ($ppn > 0) {
            // 🔥 Get OCLY akun for PPN Keluaran
            $akun_ppn_keluaran = $this->M_akunbiaya->get_by_kode('53');

            if ($akun_ppn_keluaran) {
                $entries[] = [
                    'akun_id' => $akun_ppn_keluaran->id,
                    'debit' => 0,
                    'kredit' => $ppn  // Hutang PPN ke negara
                ];
            } else {
                log_message('error', 'OCLY Akun 53 (PPN Keluaran) tidak ditemukan!');
            }
        }

        // 4. DEBIT: PPH Dipotong (OCLY 54) - jika ada PPH
        if ($pph > 0) {
            // 🔥 Get OCLY akun for PPH Dipotong
            $akun_pph_dipotong = $this->M_akunbiaya->get_by_kode('54');

            if ($akun_pph_dipotong) {
                $entries[] = [
                    'akun_id' => $akun_pph_dipotong->id,
                    'debit' => $pph,  // Asset, bisa dikreditkan
                    'kredit' => 0
                ];
            } else {
                log_message('error', 'OCLY Akun 54 (PPH Dipotong) tidak ditemukan!');
            }
        }
        ...
        if ($is_payment) {
            $this->M_tagihan_customer->update($tagihan_id, [
                ...
                'ppn' => $ppn,  // Update PPN di tagihan
                'pph' => $pph,  // Update PPH di tagihan
                'total_tagihan' => $nominal + $ppn - $pph,  // Update total
    ...
    // 🔥 REPLACE fungsi proses_ubah() di Pemasukan controller dengan ini:
    ...
    public function proses_ubah()
    {
        ...
        // 🔥 CREATE NEW JOURNAL ENTRIES WITH OCLY SUPPORT
        $entries = [];
        ...
        } elseif ($new_tagihan_id) {
            // Same tagihan, just update amounts
            $this->M_tagihan_customer->update($new_tagihan_id, [
                'ppn' => $ppn,
                'pph' => $pph,
                'total_tagihan' => $nominal + $ppn - $pph,
    ...
    // 🔥 ADD/REPLACE fungsi hapus() di Pengeluaran controller dengan ini:
    ...
    public function hapus($id)
    {
        ...
        // Delete by referensi Pengeluaran
        $this->M_transaksi_keuangan->delete_by_referensi('Pengeluaran', $id);

        // If this is payment for tagihan vendor, also delete payment journal
        if ($pengeluaran->tagihan_id) {
            $this->M_transaksi_keuangan->delete_by_referensi('Pembayaran_Vendor', $pengeluaran->tagihan_id);
        }

        // 🔥 STEP 2: Reset tagihan vendor status (if linked)
        if ($pengeluaran->tagihan_id) {
            ...
            log_message('info', "Reset tagihan_vendor #{$pengeluaran->tagihan_id} status to Waiting Payment");
        }

        // 🔥 STEP 3: Delete pengeluaran record
        if ($this->M_pengeluaran->delete($id)) {
            $msg = "Pengeluaran berhasil dihapus";

            if ($pengeluaran->tagihan_id) {
                $msg .= " dan status tagihan vendor telah direset ke Waiting Payment";
            }

            $this->session->set_flashdata('success', $msg);
            log_message('info', "Pengeluaran #{$id} deleted successfully. Reff: {$pengeluaran->reff_no}");
        } else {
            $this->session->set_flashdata('error', 'Gagal menghapus data');
            log_message('error', "Failed to delete pengeluaran #{$id}");
        }
